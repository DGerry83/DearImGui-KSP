...
using UnityEngine;
using ILogger = DearKSP.Application.Interfaces.ILogger;
...
namespace DearKSP.Infrastructure
{
    ...
    internal sealed class NativeBridge : INativeBridge
    {
        ...
        public int Initialize()
        {
            ...
            {
                _logger.Error("Native/managed handshake mismatch: expected version " + ExpectedNativeVersion + ", DearKSPNative reported " + nativeVersion + " (spec §5.4).");
                Unload();
            ...
            if (contextResult != 0)
            {
                _logger.Error("DearKSPNative_ContextInit failed with code " + contextResult + ".");
                Unload();
