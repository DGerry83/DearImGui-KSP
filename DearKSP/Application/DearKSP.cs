...
using DearKSP.Interop;

namespace DearKSP
{
    /// <summary>
    /// Public consumer-facing API for Dear KSP (spec §5.4, §6.1; locked in C7).
    ...
    public static class DearKSP
    {
        ...
        public static void Register(string id, Action callback)
        {
            ...
            if (!IsAvailable)
            {
                Log?.Warn("Register('" + id + "') ignored: Dear KSP is not available.");
                return;
            }
        ...
        public static bool Unregister(string id)
        {
            if (!IsAvailable)
            {
                Log?.Warn("Unregister('" + id + "') ignored: Dear KSP is not available.");
                return false;
            }
