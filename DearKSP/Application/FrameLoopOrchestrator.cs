...
namespace DearKSP.Application
{
    ...
    /// <see cref="DearKSP.IsAvailable"/> flag flipped by Composition after bridge init.
    /// </summary>
    internal sealed class FrameLoopOrchestrator
    {
        ...
        internal void RunFrame(float width, float height, float deltaTime)
        {
            ...
            foreach (ConsumerRegistry.ConsumerRegistration consumer in _registry.Ordered)
            {
                ...
                {
                    DearKSP.Log?.Error("Consumer '" + consumer.Id + "' threw an exception: " + ex);
                }
            }
            _bridge.EndUiFrame();
        }
