# Planning Worksheet: DearImGui-KSP

## Date: 2026-07-29

## Design Spec Reference: [DESIGN_SPEC.md](../2026-07-29_DesignSpec_DearImGuiKSP_UI_Library/DESIGN_SPEC.md)

### Step 1 — Core Entities and State

| Entity | Identity | Attributes | Relationships | Lifetime |
|--------|----------|------------|---------------|----------|
| LibrarySettings | Singleton config (one per install) | formatVersion:int, uiScale:float, fontScale:float, theme:string, verboseLogging:bool, enabled:bool | Read/written by SettingsStore; consumed by frame loop and atlas | Persistent (`GameData/DearImGuiKSP/settings.cfg` ConfigNode) |
...
| DearImGuiKSP (public facade) | Exposes availability, consumer registration, and style access to consumer mods. | Both | Application (managed public API) |
...
| DearImGuiKSPAddon | Is the KSP entry point and composition root that wires all components at startup. | Command | Infrastructure |
...
| DearImGuiKSPLogger | Writes `[DearImGuiKSP]`-prefixed lines to KSP.log with verbosity control. | Command | Infrastructure |
...
[KSPAddon MainMenu] --(start signal)--> DearImGuiKSPAddon --(concrete instances)--> Composition root --(wired components)--> LifecycleStateMachine
...
| ILogger | Application | DearImGuiKSPLogger (Infrastructure) | All Application components | Single `[DearImGuiKSP]` logging seam with verbosity control |
...
|---|---|---|
| Third-party mods extend the library at runtime | Plugin / Mod Architecture | DearImGui-KSP is a host defining a public API + registration hook; KSP's loader is the plugin loader; consumer lifecycle = register → per-frame callback → auto-disable on repeated exceptions. |
...
DearImGui-KSP/
├── DearImGuiKSP.slnx
├── DearImGuiKSP.props.user              # gitignored: pins KSPBT_GameRoot
├── .gitignore
├── README.md
├── AGENTS.md
├── DearImGuiKSP/                        # managed plugin — Application + Infrastructure layers
│   ├── DearImGuiKSP.csproj              # SDK-style, net48, KSPBuildTools
│   ├── DearImGuiKSP.version             # AVC version template
│   ├── LibraryConfig.cs            # constants: mod name, paths, failure threshold, defaults
│   ├── Properties/
│   │   └── AssemblyInfo.cs         # [assembly: KSPAssembly("DearImGuiKSP", ...)]
│   ├── Application/
│   │   ├── README.md
│   │   ├── DearImGuiKSP.cs              # public consumer-facing facade
│   │   ├── ConsumerRegistry.cs
...
│   └── Infrastructure/
│       ├── README.md
│       ├── DearImGuiKSPAddon.cs         # KSPAddon entry point
...
├── DearImGuiKSPNative/                  # C++ Core layer (no KSP/Unity knowledge)
│   ├── README.md
│   ├── src/
│   │   └── DearImGuiKSPNative.cpp       # plugin export surface stub
...
Logging: KSP.log via `UnityEngine.Debug.Log` wrapper, `[DearImGuiKSP]` prefix, plain text; levels = error/warning/info always, debug gated by `verboseLogging`.
...
|---|-----------|------------|--------------|------------------|-------------------------------|
| 1 | Build pipeline + deployment | slnx, both csproj, native build scripts, addon stub | `dotnet build` + native `build.bat`; launch KSP | Both DLLs deploy into ReformTestInstance `GameData/DearImGuiKSP/`; KSP.log shows `[DearImGuiKSP]` startup line | G1 |
...
| 3 | Consumer API + core widgets | DearImGuiKSP facade, ConsumerRegistry, cimgui bindings for MVP widgets, DemoConsumer | Demo mod in-game | Demo window with all MVP widgets via C# API only, zero IMGUI (AC3); separate consumer loads after library via EqualMajor dependency (AC4) | G3 |
