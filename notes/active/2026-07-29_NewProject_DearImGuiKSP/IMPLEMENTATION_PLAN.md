# Implementation Plan: DearImGui-KSP

## 1. Overview

### 1.1 Project Name

DearImGui-KSP (assembly/namespace: `DearImGuiKSP`).
...
- Distribution: release zip with `GameData/DearImGuiKSP/` (managed + native DLLs, `settings.cfg`, LICENSE, README, version file); demo mod as a separate zip.
...
- Attributes:
  - mouseCaptured: bool — pointer over a DearImGui-KSP window
...
### Component: DearImGuiKSP (public facade)
...
- Layer: Infrastructure
- Type: Service
- Collaborators: DearImGuiKSPLogger
...
### Component: DearImGuiKSPLogger
- Responsibility: Writes `[DearImGuiKSP]`-prefixed lines to KSP.log with verbosity control.
- Layer: Infrastructure
- Type: Service
...
[KSPAddon MainMenu] --(start)--> DearImGuiKSPAddon --(wiring)--> Composition --(components)--> LifecycleStateMachine --(Running)--> FrameLoopOrchestrator
[DearImGuiKSPNative.dll] --(LoadLibrary)--> NativeBridge --(handles)--> ImGuiContextHost --(RenderContext)--> RenderBackend
...
### Interface: ILogger
- Defined in layer: Application
- Implemented by: DearImGuiKSPLogger (Infrastructure)
- Consumed by: all Application components
- Methods:
  - Error(message) / Warn(message) / Info(message) / Debug(message): `[DearImGuiKSP]`-prefixed KSP.log lines; Debug gated by verboseLogging
...
DearImGui-KSP/
├── DearImGuiKSP.slnx
├── DearImGuiKSP.props.user                    # gitignored local KSP install pin
├── .gitignore / README.md / AGENTS.md
├── DearImGuiKSP/
│   ├── DearImGuiKSP.csproj
│   ├── DearImGuiKSP.version
│   ├── LibraryConfig.cs
...
│       ├── SettingsStore.cs
│       ├── FailureNotifier.cs
│       └── DearImGuiKSPLogger.cs
├── DearImGuiKSPNative/
│   ├── README.md
│   ├── src/DearImGuiKSPNative.cpp
│   ├── build.bat
│   └── build_release.bat
├── DearImGuiKSPDemo/
│   ├── DearImGuiKSPDemo.csproj
│   ├── DearImGuiKSPDemo.version
│   └── DemoConsumer.cs
...
|---|---|---|
| `DearImGuiKSP/LibraryConfig.cs` | Constants | Mod name, paths, failure threshold, setting defaults |
| `DearImGuiKSP/Properties/AssemblyInfo.cs` | Attributes | `KSPAssembly` self-identification |
| `DearImGuiKSP/Application/DearImGuiKSP.cs` | Facade | Public consumer API (IsAvailable, registration, style) |
| `DearImGuiKSP/Application/ConsumerRegistry.cs` | Component | Consumer registration and ordering |
| `DearImGuiKSP/Application/FrameLoopOrchestrator.cs` | Component | Per-frame sequence |
| `DearImGuiKSP/Application/InputCaptureTracker.cs` | Component | Capture-state computation |
| `DearImGuiKSP/Application/LifecycleStateMachine.cs` | Component | State transitions and gates |
| `DearImGuiKSP/Application/FaultBarrier.cs` | Component | Consumer exception containment |
| `DearImGuiKSP/Application/SettingsModel.cs` | Component | Settings snapshot and change notice |
| `DearImGuiKSP/Application/Interfaces/*.cs` | Interfaces | Cross-boundary seams (§5) |
| `DearImGuiKSP/Infrastructure/DearImGuiKSPAddon.cs` | Entry point | KSPAddon bootstrap |
| `DearImGuiKSP/Infrastructure/Composition.cs` | DI root | Concrete wiring |
| `DearImGuiKSP/Infrastructure/NativeBridge.cs` | Component | DLL load + P/Invoke isolation |
| `DearImGuiKSP/Infrastructure/InputLockGateway.cs` | Component | InputLockManager wrapper |
| `DearImGuiKSP/Infrastructure/GameEventHooks.cs` | Component | Game-event signals |
| `DearImGuiKSP/Infrastructure/SettingsStore.cs` | Component | ConfigNode persistence |
| `DearImGuiKSP/Infrastructure/FailureNotifier.cs` | Component | Failure popup |
| `DearImGuiKSP/Infrastructure/DearImGuiKSPLogger.cs` | Component | Log wrapper |
| `DearImGuiKSPNative/src/DearImGuiKSPNative.cpp` | Component | Plugin export surface (Core layer) |
| `DearImGuiKSPDemo/DemoConsumer.cs` | Demo | Example window + torture test |
...
| Consumer callback throws | try/catch in FaultBarrier | Skip frame; auto-disable at 5 consecutive | Log-only under `[DearImGuiKSP]` |
...
- Format: plain text, `[DearImGuiKSP]` prefix.
...
|---|---|---|---|---|
| 1 | Build pipeline + deployment | slnx, csproj ×2, native build scripts, addon stub, Composition | `dotnet build` + `build.bat`, launch KSP | DLLs deploy to ReformTestInstance; `[DearImGuiKSP]` startup line in KSP.log |
...
| 3 | Consumer API + core widgets | DearImGuiKSP facade, ConsumerRegistry, MVP widget bindings, DemoConsumer | Demo mod in-game | All MVP widgets via C# API, zero IMGUI (AC3); EqualMajor consumer load order (AC4) |
