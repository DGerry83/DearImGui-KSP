...
| G4 | Layered project skeleton exists (`src/Core`, `src/Application`, `src/Infrastructure`, `tests/`) | Directory tree inspected — adapted to stack: Core=`DearImGuiKSPNative/`, Application/Infrastructure=`DearImGuiKSP/`, `tests/` present; `dotnet build` clean, native `build.bat` compiles, deploy into ReformTestInstance confirmed | PASS |
