# Integration Contract: DearImGui-KSP Implementation
...
| Widget bindings | Internal-only cimgui P/Invoke for MVP widgets behind safe C# types; no raw pointers escape `DearImGuiKSP/Interop/` | C6 | C7 |
| Public facade | `DearImGuiKSP.IsAvailable`, `Register(string id, Action callback)`, `Unregister(string id)`; additive-only within a major version | C7 | C8 and all external consumers |
...
|-------------|--------------|
| C1 | `dotnet build` clean; KSP launch shows `[DearImGuiKSP]` line in KSP.log |
