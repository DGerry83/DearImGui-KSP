...
- New: `DearImGuiKSPNative/src/ContextHost.h`, `DearImGuiKSPNative/src/ContextHost.cpp` — the C ABI + implementation.
- New: `DearImGuiKSPNative/harness/` + `DearImGuiKSPNative/build_harness.bat` — console smoke test (CinematicRecorder convention).
- Modified: `DearImGuiKSPNative/build.bat`, `DearImGuiKSPNative/build_release.bat` — add imgui/cimgui TUs, new sources, include paths.
- `DearImGuiKSPNative/src/DearImGuiKSPNative.cpp` — only if wiring is needed to expose context-host exports; keep changes minimal.

### Context Host C ABI (locked — C4/C5 consume this)

| Export | Signature | Purpose |
|--------|-----------|---------|
| `DearImGuiKSPNative_ContextInit` | `int (void)` — 0 on success, nonzero on failure | Creates the ImGui context, applies stock dark style, builds the ProggyClean atlas |
| `DearImGuiKSPNative_ContextShutdown` | `void (void)` | Destroys the context and frees atlas CPU data |
| `DearImGuiKSPNative_BeginFrame` | `void (float width, float height, float deltaSeconds)` | Sets display size/delta, calls `ImGui::NewFrame()` |
| `DearImGuiKSPNative_EndFrame` | `void (void)` | Calls `ImGui::Render()`; draw data remains available to backends |
| `DearImGuiKSPNative_GetFontAtlasPixels` | `int (unsigned char** outPixels, int* outWidth, int* outHeight)` — 0 on success | RGBA32 atlas for backend texture upload (C4/C5) |
| `DearImGuiKSPNative_SetDemoWindowVisible` | `void (int visible)` | Toggles the ImGui demo window for the PoC |
...
- `build.bat` compiles 0 errors / 0 warnings with all imgui/cimgui TUs; DLL deploys to `GameData/DearImGuiKSP/PluginData/`.
...
- `dotnet build DearImGuiKSP.slnx` still clean (no managed edits).

### Rollback

- `git checkout -- DearImGuiKSPNative/` and delete untracked new files under `DearImGuiKSPNative/`.
