...
- New: `DearKSPNative/include/IUnityGraphicsD3D11.h` (verbatim copy).
- New: `DearKSPNative/src/BackendD3D11.h/.cpp` — device-event handling, backend init/shutdown, render-event rendering.
- Modified: `DearKSPNative/src/DearKSPNative.cpp` — wire backend into plugin load/unload and the render-event callback.
- Modified: `DearKSPNative/build.bat`, `build_release.bat` — add `imgui_impl_dx11.cpp` + `BackendD3D11.cpp`, link `d3d11.lib dxgi.lib`.
- Modified: `DearKSP/Application/Interfaces/INativeBridge.cs` — the locked method set (below).
- Modified: `DearKSP/Infrastructure/NativeBridge.cs` — real implementation.
- Modified: `DearKSP/Infrastructure/Composition.cs`, `DearKSP/Infrastructure/DearKSPAddon.cs` — wire the frame pump.
...
- Managed loads the DLL with `SetDllDirectory(<KSP root>/GameData/DearKSP/PluginData)` + `LoadLibrary("DearKSPNative.dll")`, then `GetProcAddress` + `Marshal.GetDelegateForFunctionPointer` for every native function. No implicit `[DllImport]` (documented GameData load-path gotcha).
- Handshake: managed expects `DearKSPNative_GetVersion() == 1` for the 0.1.x line; mismatch → error log, no init (popup arrives in C13).
- Device gate: `DearKSPNative_GetGraphicsDeviceKind()` must equal `kUnityGfxRendererD3D11` (2); anything else → log and no init for this PoC (GL arrives in C5).
...
- Deploy to ReformTestInstance (both build steps), user launches KSP:
  ...
  - KSP.log shows `[DearKSP]` init lines and no exceptions.

### Rollback

- `git checkout -- DearKSPNative/ DearKSP/` and delete untracked files under both. **If the PoC itself fails in-game: stop the entire sequence, do not patch around it — report to the user for the uGUI-fallback decision (INTEGRATION_CONTRACT rollback plan).**
