# Plan Digest: Dear KSP Implementation

## Date: 2026-07-29
## Source Plan: `notes/active/2026-07-29_NewProject_DearKSP/IMPLEMENTATION_PLAN.md` (+ `PLANNING_WORKSHEET.md`, confirmed `DESIGN_SPEC.md`)

### Plan-Specific Invariants (filled in for this codebase — always enforced)

1. **Layer boundaries** — `DearKSP/Application/` stays Unity/KSP-free; only `DearKSP/Infrastructure/` references KSP/Unity APIs; `DearKSPNative/` has zero KSP/Unity knowledge and talks C ABI only.
2. **Public API stability** — once milestone 3 lands, the consumer-facing `DearKSP` facade is additive-only within a major version (D17).
...
|---------|---------|----------------------|------------|
| M1 Build pipeline | slnx/csproj/native scripts verified; in-game startup log line | All skeleton files; `DearKSPAddon.cs` | Low |
| M2 Render-injection PoC | Unity plugin exports, ImGui context, D3D11+GL backends, minimal NativeBridge | `DearKSPNative/src/*`, `NativeBridge.cs`, cimgui/imgui sources | **High** (unproven pattern) |
| M3 Consumer API + widgets | DearKSP facade, registry, cimgui bindings for MVP widgets, demo window | `Application/*.cs`, `DearKSPDemo/*` | Med |
...
| M6 Benchmark + compat | Torture-test UI, instrumentation, full mod environment | `DearKSPDemo/*` | Low |

### Files Requiring Changes

| File | Change Type | Plan Section | Depends On |
|------|-------------|--------------|------------|
| `DearKSPNative/src/DearKSPNative.cpp` | Modify | M2 | M1 |
| `DearKSPNative/src/` (new: context host, backends, atlas, cimgui/imgui TUs) | Add | M2 | M1 |
| `DearKSPNative/build*.bat` | Modify (add imgui/cimgui sources) | M2 | M1 |
| `DearKSP/Infrastructure/NativeBridge.cs` | Modify | M2 | M1 |
| `DearKSP/Infrastructure/DearKSPLogger.cs`, `Composition.cs` | Modify | M1 | — |
| `DearKSP/Application/*.cs` (facade, registry, orchestrator) | Modify | M3 | M2 |
| `DearKSPDemo/DemoConsumer.cs` | Modify | M3, M6 | M3 |
| `DearKSP/Application/InputCaptureTracker.cs`, `Infrastructure/InputLockGateway.cs`, `Application/FaultBarrier.cs` | Modify | M4 | M3 |
| `DearKSP/Application/SettingsModel.cs`, `LifecycleStateMachine.cs`, `Infrastructure/{SettingsStore,GameEventHooks,FailureNotifier}.cs` | Modify | M5 | M3 |
...
| KSP test instance `C:\SSDGames\ReformTestInstance` | Yes | Present; DearKSP + DearKSPDemo already deploy there |
