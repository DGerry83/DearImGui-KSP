...
| C4 | Rebuilt after fix, awaiting in-game AC1 re-check | `include/IUnityGraphicsD3D11.h`, `src/BackendD3D11.{h,cpp}` (new); `src/DearKSPNative.cpp`, `build*.bat`, `INativeBridge.cs`, `NativeBridge.cs`, `Composition.cs`, `DearKSPAddon.cs`, `LibraryConfig.cs`, `Application/InputCaptureState.cs` (new) | native + managed 0 err/0 warn; harness PASS; deployed 22:15 | G3 (C4): PASS; G6 (AC1): PENDING user run | First in-game run failed (code 5): Unity never calls UnityPluginLoad for LoadLibrary'd plugins → device capture switched to texture->GetDevice pattern (CinematicRecorderNative precedent); device gate moved to managed SystemInfo |
| C5 | Pending | - | - | - | - |
| C6 | Pending | - | - | - | - |
| C7 | Pending | - | - | - | - |
| C8 | Pending | - | - | - | - |
