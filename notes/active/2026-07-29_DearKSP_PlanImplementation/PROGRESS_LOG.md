...
| C4 | Built, awaiting in-game AC1 check | `include/IUnityGraphicsD3D11.h`, `src/BackendD3D11.{h,cpp}` (new); `src/DearKSPNative.cpp`, `build*.bat`, `INativeBridge.cs`, `NativeBridge.cs`, `Composition.cs`, `DearKSPAddon.cs`, `LibraryConfig.cs`, `Application/InputCaptureState.cs` (new) | native + managed 0 err/0 warn; harness PASS; deployed 22:00 | G3 (C4): PASS; G6 (AC1): PENDING user run | 3 design-note corrections by sub-agent, all sound (see Decisions); kill-or-continue gate — in-game result decides |
| C5 | Pending | - | - | - | - |
| C6 | Pending | - | - | - | - |
| C7 | Pending | - | - | - | - |
| C8 | Pending | - | - | - | - |
