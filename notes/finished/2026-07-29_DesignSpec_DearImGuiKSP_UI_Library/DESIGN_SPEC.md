# Design Specification: DearImGui-KSP

## 1. Overview

### 1.1 Project Name

**DearImGui-KSP** (no underscore). Assembly name and root namespace: `DearImGuiKSP`.
...
Release zip containing `GameData/DearImGuiKSP/` with:

- `Plugins/DearImGuiKSP.dll` — managed C# assembly.
- `PluginData/DearImGuiKSPNative.dll` — native C++ assembly (x64 Windows; Linux/Mac binaries join later). Native DLLs must stay out of `Plugins/`: KSP's assembly loader attempts to load every DLL in its scan path as a managed assembly and hangs on native ones (D19).
- `settings.cfg` — default settings ConfigNode.
- `LICENSE`, `README.md`, and a version file.

Install = extract into the KSP root. CKAN metadata deferred to release time. The demo/example mod ships as a **separate zip** (`GameData/DearImGuiKSPDemo/`) so users installing the library purely as a dependency receive no demo UI (D7).
...
3. Another mod can declare `KSPAssemblyDependency` on DearImGui-KSP and load correctly (after it, per the loader's topological sort).
...
- Managed C# wrapper (`DearImGuiKSP.dll`): KSP plugin lifecycle, library-owned frame loop, consumer registration API, input-lock integration, theming/styling API surface, settings persistence, failure handling.
- Global library config persisted as a ConfigNode under `GameData/DearImGuiKSP/`.
...
- Failure detection with session-permanent self-disable, player popup, and `DearImGuiKSP.IsAvailable` consumer escape hatch.
...
| Application (managed) | C# in `DearImGuiKSP.dll` | Consumer registration, frame-loop orchestration, input-lock policy, lifecycle state machine, settings model. Reaches Core only through the cimgui P/Invoke boundary. |
| Infrastructure (managed) | C# in `DearImGuiKSP.dll` | KSP addon entry point, `InputLockManager` calls, ConfigNode persistence, logging, failure popup. The only layer that touches KSP/Unity APIs. |
...
- The library persists **only its own global config** (theme, UI scale, font scale, logging, enable flag) in `GameData/DearImGuiKSP/settings.cfg`, a KSP `ConfigNode`. Read at startup, written on change; no file-watching in MVP. The ConfigNode carries a format version so old configs migrate forward.
...
|---|---|---|
| Third-party mods extend the library at runtime | Plugin / Mod Architecture | DearImGui-KSP is a host: it defines a public API + registration hook; KSP's loader is the plugin loader; consumer lifecycle = register → per-frame callback → auto-disable on repeated exceptions. |
...
- Consumers can query `DearImGuiKSP.IsAvailable` before registering to fall back to their own UI.
...
|---------|-----------|--------------|-------|
| Failure popup title | DK_FailTitle | DearImGui-KSP — Startup Failed | Title case; shown by stock `PopupDialog` at main menu |
| Failure body — native component | DK_FailNative | DearImGui-KSP could not start because its native component is missing or corrupt. Mods that depend on DearImGui-KSP will not work. See KSP.log for details. | Reinstall is the expected remedy |
| Failure body — graphics API | DK_FailGraphics | DearImGui-KSP could not start because this graphics API is not supported. Mods that depend on DearImGui-KSP will not work. See KSP.log for details. | Log states the detected API |
| Failure body — version mismatch | DK_FailVersion | DearImGui-KSP could not start because its components are from different versions. Mods that depend on DearImGui-KSP will not work. See KSP.log for details. | Usually a partial update |
| Log prefix | DK_LogPrefix | [DearImGuiKSP] | Prepended to all library log lines; technical detail lives here |
...
| Consumer auto-disabled after 5 throwing frames | Log-only: consumer name, exception, and disable notice under `[DearImGuiKSP]` |
...
| DK_ManagedDll | Assembly | `GameData/DearImGuiKSP/Plugins/DearImGuiKSP.dll` | Authored in-repo | .NET assembly (x64 Mono) | Required |
| DK_NativeDll | Assembly | `GameData/DearImGuiKSP/Plugins/DearImGuiKSPNative.dll` | Authored in-repo | Native x64 Windows DLL | Required |
| DK_Settings | Config | `GameData/DearImGuiKSP/settings.cfg` | Authored in-repo | KSP ConfigNode | Required |
| DK_Demo | Code | `GameData/DearImGuiKSPDemo/` | Authored in-repo | C# only, no assets | Required (separate install) |
...
All settings live in `GameData/DearImGuiKSP/settings.cfg` (ConfigNode, format-versioned; read at startup, written on change).
...
| verboseLogging | Toggle | false | true/false | Extra diagnostic logging under `[DearImGuiKSP]` |
...
None for MVP. Consumer mods hard-depend on DearImGui-KSP via `KSPAssemblyDependencyEqualMajor("DearImGuiKSP", x, y)` (see §10.5).
...
| AC4 | A separate consumer declaring `KSPAssemblyDependencyEqualMajor("DearImGuiKSP", x, y)` loads after the library and works | Two-mod install test; load order confirmed in log |
