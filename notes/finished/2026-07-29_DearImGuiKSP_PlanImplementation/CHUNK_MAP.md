# Chunk Map: DearImGui-KSP Implementation
...
|----|------|------|---------------|-----------------|-----------|--------|
| C1 | M1 close-out: logger, composition wiring, in-game log line | Cleanup | `DearImGuiKSPLogger.cs`, `Composition.cs`, `DearImGuiKSPAddon.cs` | Low | M1 | Pending |
| C2 | Native plugin exports + device detection | Foundation | `DearImGuiKSPNative/src/` (plugin entry, Unity interface structs) | Med | M2 | Pending |
...
| C6 | cimgui interop layer (MVP widgets) | Foundation | `DearImGuiKSP/Interop/` (internal P/Invoke bindings) | Med | M3 | Pending |
| C7 | Consumer API: facade, registry, frame loop | Vertical slice | `DearImGuiKSP.cs`, `ConsumerRegistry.cs`, `FrameLoopOrchestrator.cs`, `Composition.cs` | Med | M3 | Pending |
...
| C14 | Torture-test benchmark + instrumentation | Vertical slice | `DearImGuiKSPDemo/` benchmark window | Med | M6 | Pending |
...
| C7 | C8+ | Public facade: `DearImGuiKSP.IsAvailable`, `Register(id, callback)`, `Unregister(id)` | Consumer contract; additive-only thereafter (invariant 2) |
...
|-----------|---------------------|-------------------|
| M1 Build pipeline | C1 | `[DearImGuiKSP]` startup line in KSP.log |
