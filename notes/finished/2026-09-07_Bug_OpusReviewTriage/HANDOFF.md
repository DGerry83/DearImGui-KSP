...
Gate B3 PASS (IMGUI virtualization toggle sticks; telemetry window shows Graphs + Orbit only). Temporary scaffolding removed (commit `1fd157c`). Gate D complete: 1.0.1 released — versions bumped in lockstep, CHANGELOG entry, `package_release.bat` green end-to-end, both zips inspected (library: no settings.cfg, License/Docs/Textures present; demo: DLL+.version+License+Readme), Release build + 178/178 tests. PDB decision: kept OUT of the zips (compressed ~20 MB vs the 2 MB library zip; KSP never loads them) — `package_release.bat` now archives the versioned native PDB to `dist\symbols\` for dev-side crash symbolication. Clean-KSP zip-install check handed to the user as the final D16-convention step. **Result: all PASS** (2026-09-08) — library zip installs clean on a stock KSP 1.12.x (no popup, no `[DearImGuiKSP]` errors), demo zip works in flight (demo window, benchmark, plots, control panel), and `settings.cfg` was recreated at runtime with settings persisting across a restart. The remediation wave is fully closed.
